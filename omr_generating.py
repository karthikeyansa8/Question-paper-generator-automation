...
ORDER BY stud_d.roll_number ASC
    ...
    def insert_data_to_the_DB(self,student):
        ...
        Args:
            student (Dataframe): _description_
        """        
        ...
        """
            if student is not None :
                ...
                track_id = db.execute_query_and_return_result(query=insert_query,vars = (1, 'Student', None,None,None, self.school_id, 'Generated', 'False', 'False', 47))
                

                #self.cur.execute(insert_query,(1,'Student',None,None,None,self.school_id,'Generated','True','False',47))
    ...
    def update_some_column_in_DB(self,student):
        """_summary_
        ...
        try:
            ...
            if student is not None:
                ...
                db.execute_query(query=update_query,vars=(os.path.basename(self.pdf_filename),self.presigned_url,None,self.exam_doc_track_id))
                #self.cur.execute(update_query,(os.path.basename(self.pdf_filename),self.presigned_url,None,self.exam_doc_track_id))
    ...
    def create_latex_document(self, student, omr_type,  error=True):
        ...
        Args:
            ...
            omr_type (int):  for QR code and barcode names
        ...
        if student and isinstance(student, dict):
            ...
            qr_data = self.exam_doc_track_id
            barcode_data =  str(self.exam_doc_track_id)

            self.generate_qr_code(qr_data, self.serial_number)
            self.generate_barcode(barcode_data, self.serial_number)
        ...
        if student and isinstance(student, dict):
            ...
            doc = self.add_student_commands(doc, None)

        if student and isinstance(student, dict):
            doc = self.add_omr_page_content(doc,student)
        else:
            doc = self.add_omr_page_content(doc,None)


        if student and isinstance(student, dict):
            doc = self.add_student_particular(doc,student)
        else:
            doc = self.add_student_particular(doc,None)

        doc = self.add_bubles_content(doc)

        mul_folder = self.create_mul_folder(student) if student else self.output_folder

        self.pdf_filename = os.path.join(self.output_folder, f'omr_sheet_{student["lapis_roll_number"]}-{self.code}' if student else f'omr_sheet_{self.school_id}_{self.exam_doc_track_id}')
        ...
        # Move the without roll number pdf to the empty doc folder
        if not student: 
            
            empty_folder = os.path.join(self.output_folder, 'empty_documents')
            
            os.makedirs(empty_folder, exist_ok=True)  
            try:
                
                shutil.move(f'{self.pdf_filename}.pdf', os.path.join(empty_folder))  
                
            except Exception as e:
                print(e)
            
            print(f"Moved empty document to {empty_folder}")

            self.upload_pdf_to_minio(empty_folder,student)
        else:
    ...
    def upload_pdf_to_minio(self, d_path, student):
        ...
        if student is not None:
            ...
            self.last_section_folder = section_folder
        
        else:
             # Use the last known section folder if available
            
            source_file = f'{d_path}/omr_sheet_{self.school_id}_{self.exam_doc_track_id}.pdf' 
            minio_pdf_path = f'{self.last_section_folder}omr_sheet_{self.school_id}_{self.exam_doc_track_id}.pdf' 
        ...
        try:
            ...
            print(f"Successfully uploaded {os.path.basename(source_file)} to  MinIO.")
    ...
    def generate_qr_code(self, data, roll_number):
        ...
        Returns:
            _type_: _description_
        """
        qr_filename = os.path.join(self.output_folder, f'qr{roll_number}-{self.exam_doc_track_id}.png')
    ...
    def add_student_commands(self, doc, student):
        ...
        if student and isinstance(student, dict): 
            ...
            doc.preamble.append(NoEscape(r'\newcommand{\QRcodePath}{%s}' %f'qr{student["lapis_roll_number"]}-{self.exam_doc_track_id}.png'))
            ...
            doc.preamble.append(NoEscape(r'\newcommand{\rowSpacing}{0.69cm}'))  
            doc.preamble.append(NoEscape(r'\newcommand{\omrMarker}{../omr_marker.png}'))
            doc.preamble.append(NoEscape(r'\newcommand{\omrMarkerWidth}{1cm}'))
            doc.preamble.append(NoEscape(r'\newcommand{\bubbleSpacing}{0.6}'))
            doc.preamble.append(NoEscape(r'\newcommand{\maxQuestions}{60}'))
            doc.preamble.append(NoEscape(r'\newcommand{\OMRID}{87654321}'))
            doc.preamble.append(NoEscape(r'\newcommand{\QRcodePath}{%s}' %f'qr{self.serial_number}-{self.exam_doc_track_id}.png'))
            doc.preamble.append(NoEscape(r'\newcommand{\BarcodePath}{%s}' %f'barcode{self.serial_number}-{self.exam_doc_track_id}'))
            doc.preamble.append(NoEscape(r'\newcommand{\studentName}{}'))
            doc.preamble.append(NoEscape(r'\newcommand{\rollNo}{}'))
            doc.preamble.append(NoEscape(r'\newcommand{\schoolName}{}'))
            doc.preamble.append(NoEscape(r'\newcommand{\sectionClass}{}'))
            doc.preamble.append(NoEscape(r'\newcommand{\examDate}{}'))
            doc.preamble.append(NoEscape(r'\newcommand{\QPCode}{}'))
...
\textbf{Name} : &  & \multicolumn{2}{l}{\textbf{Roll Number} :}&   & \multirow{4}{*}{\includegraphics[width=3.6cm]{\QRcodePath}} \tabularnewline
\textbf{School Name} : &  & \multicolumn{2}{l}{\textbf{Class} :} &  & \tabularnewline
\textbf{Question Paper Code} : &  & \multicolumn{2}{l}{\textbf{Exam Date} :} &  & \tabularnewline
\textbf{Student Sign} : & &   \multicolumn{2}{l}{\textbf{Invigilator Sign} :} &  \tabularnewline
\end{tabular}
...
"""))
    ...
    def generate_omr_sheets(self,omr_type):
        """_summary_
        ...
        try:
            ...
            for idx,student in df.iterrows():
                ...
                try:
                    ...
                    print('-------------------')
                    self.update_some_column_in_DB(student)
                

            for j in range(2):
            #   self.code = qp_codes_sub[j % len(qp_codes_sub)]
                try:
                    self.create_latex_document(student=None, omr_type=omr_type, error=True)  
                except Exception as e:
                    self.insert_data_to_the_DB(student=None)
                    print('Finished inserting the Empty pdf data')
                    print("Generating document without particulars")
                    self.create_latex_document(student=None, omr_type=omr_type, error=False)
                    print('-------------------')
                    self.update_some_column_in_DB(student=None)
...
if __name__ == "__main__":
    ...
    omr_generator.generate_omr_sheets(omr_type="01")
