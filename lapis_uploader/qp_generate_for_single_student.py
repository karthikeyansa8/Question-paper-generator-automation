...
class QuestionPaper:
    def __init__(self,qp_code,school_id, section_id ,is_final=True) -> None:
      ...
      self.school_name,self.lapis_roll_number,self.std,self.subject,self.subject_id,self.is_interest_group = data_for_qp_from_data_base[0]
      
      self.folder_name = f"overleaf_files/question_papers/"
   
      print(self.school_name,self.std,self.subject,self.subject_id,self.is_interest_group)
...
\vspace{1mm}
    ...
    def update_the_column_in_db(self,link_data):
       ...
       try:
          ...
          db.execute_query(query,(link_data,self.exam_doc_track_id))
    ...
    def generate_question_paper(self,row,code,is_question_shuffled,is_option_shuffled):
        ...
        self.is_interest_groups = row['is_interest_group'] 
        ...
        # After generating the pdf update the url column in db
        self.update_the_column_in_db(link_data) 
    ...
    def download_csv_from_minio(self):
       ...
       try:
        ...
        self.client.fget_object(bucket_name='lapis',object_name=f"question_papers/shuffled_basic_questions_{self.code}.csv", file_path = f"shuffled_basic_questions_{self.code}.csv")
        self.client.fget_object(bucket_name='lapis',object_name=f"question_papers/shuffled_normal_questions_{self.code}.csv", file_path = f"shuffled_normal_questions_{self.code}.csv")
        self.client.fget_object(bucket_name='lapis',object_name=f"question_papers/shuffled_critical_questions_{self.code}.csv", file_path = f"shuffled_critical_questions_{self.code}.csv")
        ...
        print(e)
        raise Exception("Failed to download csv from Minio")  

        
    def upload_to_minio(self): 
      self.client = create_client()
      try:
          self.client.fput_object(bucket_name='lapis',object_name = f"question_papers/{self.lapis_roll_number}-{self.qp_code}-{self.code}.pdf",file_path=f"lapis_uploader/{self.folder_name}/{self.lapis_roll_number}-{self.qp_code}-{self.code}.pdf")
          self.client.fput_object(bucket_name='lapis',object_name = f"question_papers/{self.lapis_roll_number}-{self.qp_code}-{self.code}.tex",file_path=f"lapis_uploader/{self.folder_name}/{self.lapis_roll_number}-{self.qp_code}-{self.code}.tex")
          
          self.pdf_link = self.client.get_presigned_url(bucket_name='lapis',method='GET',object_name=f"question_papers/{self.lapis_roll_number}-{self.qp_code}-{self.code}.pdf")
          self.tex_link = self.client.get_presigned_url(bucket_name='lapis',method='GET',object_name=f"question_papers/{self.lapis_roll_number}-{self.qp_code}-{self.code}.tex")
...
order by BASE_QUESTION_NUMBER""")
    ...
    def Student_particular(self,lapis_roll_number):
      try:
        ...
        where qpd.question_paper_code  = %s and ssd.school_id = %s and sd.section_id =%s and roll_number = %s and sub_d.subject_id =101

                  """          
          rows = db.execute_query_and_return_result(query, (self.qp_code, self.school_id, self.section_id, lapis_roll_number))    
...
if __name__ == "__main__":
    ...
    qp_code = '200'  #give as a string 
    section_id = 253
    school_id = 110
    lapis_roll_number = 'D110058'

    obj = QuestionPaper(qp_code=qp_code, is_final=is_final, school_id=school_id, section_id=section_id)

    # Retrieve student particulars for the given lapis_roll_number
    df = obj.Student_particular(lapis_roll_number)

    # Get question paper variant for the roll number
    qp_codes_sub = get_qp_varaint_by_roll_number(lapis_roll_number)
    print(qp_codes_sub)

    # Process the specific student's data if it exists in df
    if not df.empty:
        row = df.iloc[0]
        code = qp_codes_sub
        print(f"Processing roll number: {row['lapis_roll_number']}")
        
        minio_links = obj.generate(row=row,code=code,download_images=False,upload_required=True,is_question_shuffled=is_question_shuffled,is_option_shuffled=is_option_shuffled)
        print(minio_links)

    else:
        print(f"No data found for roll number: {lapis_roll_number}")
