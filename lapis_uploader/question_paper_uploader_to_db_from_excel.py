...
\makeatother
    ...
    def add_data(self):
        ...
        if len(critical_question) > 0:
            ...
            self.doc.append(NoEscape(r'''
                                    \begin{minipage}{\textwidth}
                                    '''))
            self.doc.append(NoEscape(r' \Centering \Large \textbf{Critical Thinking Questions} \vspace{0.5cm}'))
            self.doc.append(NoEscape(r'''
                                    \end{minipage}
                                    '''))
    ...
    def generate_question_paper(self):
        geometry_options = {"top": "1.5cm", "bottom" : "0.5cm", "left" : "1cm", "right" : "1cm"}
...
if __name__ == "__main__":
    ...
    school_name = "Sri Swamy International School"
    
    excel_df = pd.read_excel(f"Question Selection For Lapis.xlsx",sheet_name=f"Swami - Class {std}")
