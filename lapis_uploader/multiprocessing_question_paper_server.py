...
class QuestionPaper:
    def __init__(self,qp_code,school_id, section_id ,is_final=True) -> None:
      ...
      self.conn,self.cur  = self.conn_to_lb_db_dev()
      ...
      if self.section_id:
         query=f"""select ssd.school_name,lapis_roll_number,class,subject_name,ls.subject_id,lei.is_interest_group,1 from lapis.lapis_exam_info lei 
        join lapis.section_detail sd on sd.section_id = lei.section_id
        join lapis.lapis_subject ls on ls.subject_id = lei.subject_id 
        join lapis.school_detail ssd on ssd.school_id = sd.school_id
        join lapis.student_detail sdt on sdt.section_id = lei.section_id
        where qp_code  = {qp_code} and sd.school_id = {school_id} and sd.section_id = {section_id}
        
        """
      else:
         query=f"""select ssd.school_name,class,subject_name,ls.subject_id,lei.is_interest_group,1 from lapis.lapis_exam_info lei 
        join lapis.section_detail sd on sd.section_id = lei.section_id
        join lapis.lapis_subject ls on ls.subject_id = lei.subject_id 
        join lapis.school_detail ssd on ssd.school_id = sd.school_id
        join lapis.student_detail sdt on sdt.section_id = lei.section_id
        where qp_code  = {qp_code} and sd.school_id = {school_id} 
        """
      ...
      if self.section_id:
        self.school_name,self.lapis_roll_number,self.std,self.subject,self.subject_id,self.is_intrest_groups,self.track = data_for_qp_from_data_base[0]
      
      else:
        self.school_name,self.std,self.subject,self.subject_id,self.is_intrest_groups,self.track = data_for_qp_from_data_base[0]

      self.folder_name = f"overleaf_files/question_papers/"
   
      print(self.school_name,self.std,self.subject,self.subject_id,self.is_intrest_groups)
...
\vspace{1mm}
    ...
    def add_instuctions(self):
        ...
        # if not self.is_intrest_groups:

        if not self.subject == 'Science':
            if self.is_intrest_groups:
              self.doc.append(NoEscape(r'''
            \vspace{0.3cm}
    ...
    def conn_to_lb_db_dev(self):
       try:
          conn = py.connect(
              dbname=self.LB_DB_DEV_DATABASE_NAME,
    ...
    def insert_the_pdf_data_to_DB(self,link_data):
      try:
          

          query="""INSERT INTO exam.question_paper_detail(question_paper_code,question_paper_type,is_question_shuffled,base_question_paper_code,created_by,question_paper_pdf_link,question_paper_latex_link)
          values(%s,%s,%s,%s,%s,%s,%s) 
          ON CONFLICT (question_paper_code) DO UPDATE SET
           question_paper_type = EXCLUDED.question_paper_type,
           is_question_shuffled = EXCLUDED.is_question_shuffled,
           base_question_paper_code = EXCLUDED.base_question_paper_code,
           created_by = EXCLUDED.created_by,
           question_paper_pdf_link = EXCLUDED.question_paper_pdf_link,
           question_paper_latex_link = EXCLUDED.question_paper_latex_link
          """
          self.cur.execute(query,(f'{self.qp_code}-{self.code}','Shuffled Question Paper','true',f'{self.qp_code}-{self.code}',47,link_data['pdf_link'],link_data['tex_link']))
          self.conn.commit()
    ...
    def insert_the_questions_to_db(self, original_question_num, shuffled_question_num):
        ...
        try:
          ...
          self.shuffle_question_id = self.cur.fetchone()[0]
    ...
    def upload_to_minio(self):
      ...
      try:
          ...
          pdf_link = client.get_presigned_url(bucket_name='lapis',method='GET',object_name=f"question_papers/{self.qp_code}-{self.code}.pdf")
          tex_link = client.get_presigned_url(bucket_name='lapis',method='GET',object_name=f"question_papers/{self.qp_code}-{self.code}.tex")
...
order by BASE_QUESTION_NUMBER""")
    ...
    def connect_db(self):
      try:
          ...
          query = """
          select ssd.school_name,lapis_roll_number,class,subject_name,ls.subject_id,lei.is_interest_group,1 from lapis.lapis_exam_info lei 
        join lapis.section_detail sd on sd.section_id = lei.section_id
        join lapis.lapis_subject ls on ls.subject_id = lei.subject_id 
        join lapis.school_detail ssd on ssd.school_id = sd.school_id
        join lapis.student_detail sdt on sdt.section_id = lei.section_id
        where qp_code  = %s and sd.school_id = %s and sd.section_id = %s

                  """          
          cur.execute(query, (self.qp_code, self.school_id, self.section_id))    
          ...
          rows = cur.fetchall()
          columns = ['school_name', 'lapis_roll_number', 'class', 'subject_name', 'subject_id', 'is_interest_group', 'dummy_column']
...
if __name__ == "__main__":
    ...
    for qp_code in [905]:
      ...
      for code in qp_codes_sub:
        ...
        minio_links = obj.generate(download_images=False, upload_required=True,row=row,code=code,is_question_shuffled = is_question_shuffled,is_option_shuffled = is_option_shuffled)
    ...
    for qp_code in [905]:
        ...
        for idx, (i, row) in enumerate(df.iterrows()):
                code = qp_codes_sub[idx % len(qp_codes_sub)]
